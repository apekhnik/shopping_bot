...
| `🔥 Топ знижок` / `/top [N]` | Топ-5 товаров с самой большой скидкой в магазине юзера. Дефолтный порог 30%, кастомный — `/top 40`. Один on-demand запрос к API с `sort=…special_price_discount:desc` — без выкачки каталога |
...
Фоновая работа:
- **Скан-джоба** APScheduler каждые 12 ч опрашивает **только те SKU**,
  которые есть в чьих-то вотчлистах (`SELECT DISTINCT`), сравнивает с
  `product_state`, генерит события. Интервал настраивается через
  `SCAN_INTERVAL_SECONDS`; час — избыточно (Varus обновляет акции ~раз
  в неделю), 12 ч — комфортный дефолт.
...
- **Rate-limit Varus** не тестировался. Опрос раз в 12 ч — не агрессивно,
