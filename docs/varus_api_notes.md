...
Тот же эндпоинт умеет искать по подстроке имени через фильтр:

```json
{"attribute": "name", "value": {"like": "кокосове"}, "scope": "default"}
```

Multi-word "AND" — несколько таких фильтров подряд:

```json
[
  {"attribute": "name", "value": {"like": "кокосове"}, "scope": "default"},
  {"attribute": "name", "value": {"like": "молоко"}, "scope": "default"}
]
```

Проверено: "кокосове молоко" → 14 точных попаданий, все содержат оба слова.

**Важно: поиск case-sensitive.** `Whiskas` → 0 результатов, `whiskas` → 44.
Клиенту (боту) нужно `lower()` перед отправкой.

**Fulltext-режимы ES (`query.match`, `multi_match`, `query_string`) на этом
эндпоинте игнорируются** — бэкенд принимает JSON, но возвращает "весь
каталог". Работает только через `_appliedFilters` с `like`.

## Сортировка

**`sort` внутри тела `request` (`{"_appliedFilters":..., "sort":...}`) —
молча игнорируется.** А вот как **URL-параметр** — работает:

```
&sort=sqpp_data_57.special_price_discount:desc
```

Проверено live: с фильтром `discount >= 30%` порядок строго убывающий
(84, 82, 82, 82, 82, 80, …). Значит топ по любому индексируемому полю
берётся одним запросом без выкачки всего каталога — используется в
`/top`.

## Fetch по списку SKU (для рефреша вотчлиста)

```json
[{"attribute": "sku", "value": {"in": ["2615912", "2633576"]}, "scope": "default"}]
```

Проверено: 3 SKU → 3 hits, точный `relation: "eq"`. Можно за один
запрос вытащить сразу до ~1000 отслеживаемых товаров.
